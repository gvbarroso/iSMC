...
The last two columns are convenient to 'synchronise' coordinates when using `ismc_mapper`. This can happen when we want to restrict the final maps to a range of sites that matches that of another file. For example, if the input sequence data for chromosome 1 starts at position 5,000 and goes until position 10,000,000, and we want to 'synchronise' it with an experimental genetic map that ranges from position 10,000 to 8,000,000, the 6th and 7th columns of the first line of your tab_file should be  9,999 and 7,999,999.
...
The number of "knots" where to anchor the splines curves taming the coalescence rates over time (INTEGER). These knots are distributed in the temporal axis, and we can view them as defining the number of "epochs" (equal to the number of knots + 1). By default, they are constrained to be strictly positive, to have derivative equal to zero at each boundary and to meet the adjacent splines at the junction points (i.e., epochs show continuity of population sizes).
Inverse coalescence rates are then mapped from the splines curves at the boundaries of each discretized time interval.

If this writing seems too convoluted, note that in practice having between 2 and 3 knots is usually a good choice.
