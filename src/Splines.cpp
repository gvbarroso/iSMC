...
void Splines::includeSplinesParameters_() {
  ...
  for(size_t i = 0; i <= numberOfKnots_; ++i) {
    ...
    addParameter_(new Parameter("y" + TextTools::toString(i) + "_prime", 0., make_shared<IntervalConstraint>(-1., 1., true, true)));
  }
  //at tmax:
  addParameter_(new Parameter("y" + TextTools::toString(numberOfKnots_ + 1), 1., Parameter::R_PLUS_STAR));
  addParameter_(new Parameter("y" + TextTools::toString(numberOfKnots_ + 1) + "_prime", 0., make_shared<IntervalConstraint>(-1., 1., true, true)));
}
