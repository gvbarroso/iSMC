...
void ParameterCategoryTransitions::includeCategoryTransitionParameters_() {
  ...
  if(hetRateModel_ == "Hotspot") {
    ...
    categoryTransitions.addParameter(new Parameter(transitionIntoHotspots, backHot, new IntervalConstraint(0., 1., true, true)));
    double hotBack = 5e-4;
    string transitionIntoBackground = parameterPrefix_ + "_hotBack";
    categoryTransitions.addParameter(new Parameter(transitionIntoBackground, hotBack, new IntervalConstraint(0., 1., true, true)));
  }
  else if(hetRateModel_ == "Gamma") {
    ...
    categoryTransitions.addParameter(new Parameter(paramName, selfTransitionProb, new IntervalConstraint(0., 1., true, true)));
  }
  else if(hetRateModel_ == "Gamma+Hotspot") {
    string kappa = parameterPrefix_ + "_k"; //transition into hotspots
    double kappaVal = 2e-6;
    categoryTransitions.addParameter(new Parameter(kappa, kappaVal, new IntervalConstraint(0., 1., true, true)));
    ...
    categoryTransitions.addParameter(new Parameter(delta, deltaVal, new IntervalConstraint(0., 1., true, true)));
  }
